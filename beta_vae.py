...
class VAE(object):
  ...
  def _sample_z(self, z_mean, z_log_sigma_sq):
    ...
    with tf.compat.v1.variable_scope("reparametrize"):
      eps_shape = tf.shape(z_mean)
      #FCA/Changement dans TF2.0
  ...
  def _create_network(self):
    ...
    self.embedding_matrix = tf.compat.v1.placeholder(tf.float32, shape = self.emb_matrix_shape, name = "embedding_matrix")
  ...
  def _create_loss_optimizer(self):
    ...
    with tf.name_scope("posterior_means"):
      ...
      for index, mean in enumerate(split_means):
          ...
          single_mean, single_mean_update = tf.compat.v1.metrics.mean(mean, name=mean_name)
          ...
          self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "posterior_means/" + mean_name))
          #single_mean_summary_op = tf.summary.scalar('mean_' + mean_name , single_mean)
          single_mean_summary_op = tf.compat.v1.summary.scalar('mean_' + mean_name , single_mean)
    ...
    with tf.name_scope("posterior_variances"):
      ...
      for index, variance in enumerate(split_variances):
          ...
          single_variance, single_variance_update = tf.compat.v1.metrics.mean(variance, name=variance_name)
          ...
          self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "posterior_variances/" + variance_name))
          #single_variance_summary_op = tf.summary.scalar('variance_' + variance_name , single_variance)
          single_variance_summary_op = tf.compat.v1.summary.scalar('variance_' + variance_name , single_variance)
    ...
    with tf.name_scope("reconstr_loss"):  
      ...
      self.reconstr_loss, self.reconstr_loss_update = tf.compat.v1.metrics.mean( reconstr_loss, name = "metric")
      #self.running_vars.extend(tf.get_collection(tf.GraphKeys.LOCAL_VARIABLES, scope = "reconstr_loss/metric"))
      self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "reconstr_loss/metric"))
    ...
    with tf.name_scope("latent_loss"):
      ...
      self.latent_loss, self.latent_loss_update = tf.compat.v1.metrics.mean( total_kld, name = "metric")
      self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "latent_loss/metric"))
      ...
      self.mean_kld, self.mean_kld_update = tf.compat.v1.metrics.mean( avg_kld, name = "metric_mean_kld")
      self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "latent_loss/metric_mean_kld"))
      #mean_kld_summary_op   = tf.summary.scalar('mean_kld', self.mean_kld)
      mean_kld_summary_op   = tf.compat.v1.summary.scalar('mean_kld', self.mean_kld)
      ...
      for index, single_kld in enumerate(split_klds):
          ...
          single_kld_loss, single_kld_loss_update = tf.compat.v1.metrics.mean(single_kld, name=kld_name)
          ...
          self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "latent_loss/" + kld_name))
          single_kld_loss_summary_op = tf.compat.v1.summary.scalar('single_kld_loss_' + kld_name , single_kld_loss)
    ...
    with tf.name_scope("total_loss"):
      ...
      self.total_loss, self.total_loss_update = tf.compat.v1.metrics.mean( total_loss, name = "metric")
      self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "total_loss/metric"))
      #total_loss_summary_op    = tf.summary.scalar('total_loss',self.total_loss)
      total_loss_summary_op    = tf.compat.v1.summary.scalar('total_loss',self.total_loss)
    ...
    with tf.name_scope("accuracy"):
      ...
      self.accuracy, self.accuracy_update = tf.compat.v1.metrics.accuracy( labels = self.labels, predictions = self.predictions, name = "metric")
      self.running_vars.extend(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope = "accuracy/metric"))
      #accuracy_summary_op      = tf.summary.scalar('word_level_accuracy',self.accuracy)
      accuracy_summary_op      = tf.compat.v1.summary.scalar('word_level_accuracy',self.accuracy)
      ...
      self.running_vars_initializer = tf.compat.v1.variables_initializer(var_list = self.running_vars)
...
#          gradients = [ None if gradient is None else tf.clip_by_norm(gradient, 5.0) for gradient in gradients]
          ...
          self.gradients_global_norm = tf.compat.v1.global_norm(gradients)
    ...
    self.summary_op = tf.compat.v1.summary.merge(summary_op_list)
...
#    self.debug_summary_op            = tf.summary.merge([gradients_global_norm_summary_op]) 
  ...
  def train(self, sess, manager,saver, args):
      ...
      train_writer = tf.compat.v1.summary.FileWriter(exp_logdir +  "/train", sess.graph)
      test_writer = tf.compat.v1.summary.FileWriter(exp_logdir + "/test")
...
############################################### TRAIN ################################################
      ...
      image_summary_op = sess.run(image_summary)
      train_writer.add_summary(summary=image_summary_op,global_step=epoc+1)