...
def create_checkpoint_saver(sess, args, model):
    ...
    saver = tf.compat.v1.train.Saver(max_to_keep = args.keep_n_ckpts, save_relative_paths = True)
    ...
    tf.compat.v1.train.export_meta_graph(filename= exp_checkpointdir + "/" + "checkpoint.meta")
...
def reshape(tensor, dims_list):
  ...
  for dims in dims_list:
    if isinstance(dims, int):
      ...
      dims_prod.append(reduce_prod([shape[d] for d in dims]))
    else:
      #dims_prod.append(np.prod([shape[d] for d in dims]))
      dims_prod.append(reduce_prod([shape[d] for d in dims]))
