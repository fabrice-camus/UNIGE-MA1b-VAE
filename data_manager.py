...
class DataManager(object):
    def __init__(self, args = None):
        ...
        #FCA/Ajout allow_pickle
        self.dataset_zip = np.load(DATA_PATH+"dSentences.npz",encoding='latin1',allow_pickle=True)
        ...
        self.n_samples = self.train_samples + self.test_samples       
        print('DataManager.__init__() done !')
    ...
    def _load(self):
        ...
        #FCA / Ajout : dataset avec word2vec dim 100
        if self.repr_type == "word2vec100":
            #define string which indicates padding
            pad = "."
            #pad all sublists with the paddding string in order to make them same length
            #note that this happens in place!
            ...
            for sublist in padded_list_of_lists:
                sublist.extend( (max_len_sentence - len(sublist)) * [pad])
            original_sentences = padded_list_of_lists
            print("Loading dataset word2vec100!")
            dataset = np.load(DATA_PATH+"w2v100_dataset_dot_padded.npy")
            # now load the gensim model in order to find most similar word and reconstruct sentences
            print("Loading Embedding-Model Matrix word2vec100")
            embedding_matrix = np.load(DATA_PATH + "w2v100_embedding_matrix.npy")
            # a traiter : quelle est la structure du fichier (comment le dict a été persisté.. en array ?)
            #ces deux objets ne sont PAS utilisés semble-t-il dans l'entrainement.
            #id_to_token = np.load(DATA_PATH + "w2v100_id_to_token_dict.pkl").tolist()
            #token_to_id = np.load(DATA_PATH + "w2v100_token_to_id_dict.pkl").tolist()
            id_to_token=[]
            token_to_id=[]
